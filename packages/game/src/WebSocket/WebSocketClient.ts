import IncomingProtobuffer from "@Client/Communications/IncomingProtobuffer.js";
...
export default class WebSocketClient extends EventTarget {
    ...
    public addProtobufListener<T>(payload: { name: string; decode: (payload: Uint8Array) => T }, listener: IncomingProtobuffer<T>) {
        this.addEventListener(listener.name, (event: WebSocketEvent<any>) => {
            listener.handle(payload.decode(event.data));
        });
    }
