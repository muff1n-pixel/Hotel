...
import RoomFurniturePetFoodLogic from "../../Furniture/Logic/RoomFurniturePetFoodLogic";

export default class RoomPetEatAction implements RoomPetAction {
    expiresAt?: number;
    
    constructor(private readonly roomPet: RoomPet) {
        const waterBowlFurniture = this.roomPet.room.furnitures.filter((furniture) => furniture.logic instanceof RoomFurniturePetFoodLogic);
    ...
    private async handleFinishWalk(waterBowlFurniture: RoomFurniture) {
        ...
        if(waterBowlFurniture.model.animation === 5) {
        ...
        await this.roomPet.addExperiencePoints(5, -10);

        this.roomPet.pose.eat();

        this.roomPet.sendVocal("EATING");
