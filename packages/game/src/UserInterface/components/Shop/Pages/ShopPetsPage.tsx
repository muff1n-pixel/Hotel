...
export default function ShopPetsPage({ editMode, page }: ShopPageProps) {
    ...
    useEffect(() => {
        ...
        for(const pet of pets.toSorted((petA, petB) => (petA.pet?.palettes[0]?.paletteId ?? 0) - (petB.pet?.palettes[0]?.paletteId ?? 0))) {
    ...
    useEffect(() => {
        ...
        Promise.all(pets
            ...
            .then((values) => setActiveFilteredPets(values.filter<FilteredShopPet>((pet) => pet !== null).sort((a, b) => (a.pet.pet?.palettes[0]?.paletteId ?? 0) - (b.pet.pet?.palettes[0]?.paletteId ?? 0))));
    }, [activePet]);
