...
export default class FigureWorkerMainThread implements FigureWorkerInterface {
    public async renderInWebWorker(figure: Figure, frame: number, cropped: boolean, drawEffects: boolean, useConfigurationEffect: boolean, ignoreBodyparts: string[]): Promise<FigureRendererResult> {
        ...
        const result = await figureRenderer.renderToCanvas(cropped, drawEffects, useConfigurationEffect, ignoreBodyparts);
