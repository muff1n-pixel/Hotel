...
onmessage = async (event: MessageEvent<FigureRenderEvent>) => {
    ...
    if(event.data.type === "render") {
        ...
        const { figure, effects } = await figureRenderer.renderToCanvas(event.data.cropped, event.data.drawEffects, event.data.useConfigurationEffect, event.data.ignoreBodyparts);
