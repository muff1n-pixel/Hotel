...
export default {
    up: (queryInterface: QueryInterface): Promise<void> => queryInterface.sequelize.transaction(
        async (transaction) => {
            const furnitureDatas = await getExistingFurnitureAssets((assetName) => "wf_trg_user_performs_action" === assetName);
            ...
            await queryInterface.bulkInsert("furnitures", furnitureDatas.flatMap((furnitures) => furnitures).map((furniture) => {
                return {
                    ...
                    name: "WIRED Trigger: User Performs Action",
                    ...
                    category: "wired",
                    interactionType: "wf_trg_user_performs_action",
    ...
    down: (queryInterface: QueryInterface): Promise<void> => queryInterface.sequelize.transaction(
        async (transaction) => {
            await queryInterface.bulkDelete("furnitures", {
                where: {
                    type: "wf_trg_user_performs_action"
                },
                transaction
            });
        }
